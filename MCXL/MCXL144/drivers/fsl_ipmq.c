...
static void IPMQ_CommonIRQHandler(void)
{
    ...
    if ((flags & kMU_GeneralPurposeInterrupt14) != 0UL)
    {
        IPMQ_DisableMsgGeneralPurposeInterrupts(kMU_GeneralPurposeInterrupt14);
        if (ipmqCtx.ptrCb->extIntAssert != NULL)
        {
          ipmqCtx.ptrCb->extIntAssert();
        }
    }

    if (ipmqCtx.ptrCb->rtcAlarm0 != NULL && (flags & kMU_GeneralPurposeInterrupt3) != 0UL)
    {
        ipmqCtx.ptrCb->rtcAlarm0();
    }

    if (ipmqCtx.ptrCb->rtcAlarm1 != NULL && (flags & kMU_GeneralPurposeInterrupt4) != 0UL)
    {
        ipmqCtx.ptrCb->rtcAlarm1();
    }

    if (ipmqCtx.ptrCb->gpioIntA != NULL && (flags & kMU_GeneralPurposeInterrupt11) != 0UL)
    {
        ipmqCtx.ptrCb->gpioIntA();
    }

    if (ipmqCtx.ptrCb->gpioIntB != NULL && (flags & kMU_GeneralPurposeInterrupt12) != 0UL)
    {
        ipmqCtx.ptrCb->gpioIntB();
    }
}
 ...
 */
ipmq_ret_value_t IPMQ_SendMessage(ipmq_msg_opcode_t opcode, uint8_t userData, uint8_t payloadSize, uint32_t *ptrPayload)
{
    assert(ipmqCtx.initialized);
    ...
    /* Lock mutex. Any subsequent invocations while the mutex is locked will return a busy state. */
    if (SDK_ATOMIC_LOCAL_COMPARE_AND_SET(&(ipmqCtx.mutex), false, true) == false)
    {
        /* Return busy. */
        return kIPMQ_Ret_Busy;
    }
    ...
    /* Give sync semaphone */
    (void)SDK_ATOMIC_LOCAL_COMPARE_AND_SET(&(ipmqCtx.sync), false, true);

    return kIPMQ_Ret_SendSuccess;
}
 ...
 */
ipmq_ret_value_t IPMQ_SendMessage2Payload(ipmq_msg_opcode_t opcode,
                              uint8_t userData,
                                     ...
                                     uint8_t *ptrResponsePayloadSize)
{
    ...
    if (SDK_ATOMIC_LOCAL_COMPARE_AND_SET(&(ipmqCtx.sync), true, false) == false)
    {
        return kIPMQ_Ret_NoMsgSent;
    }
    ...
    /* Unlock mutex. */
    (void)SDK_ATOMIC_LOCAL_COMPARE_AND_SET(&(ipmqCtx.mutex), true, false);

    return ret;
}

/*!
 * brief Send message with one payload buffer and then waiting receive message from AON.
 * 
 * param[in] opcode Message opcode, please refer to ipmq_msg_opcode_t.
 * param[in] sendSize  Size of payload to send.
 * param[in] ptrSendBuffer Pointer to the buffer of payload to send.
 * param[out] ptrRcvBuffer Pointer to the buffer to store response message's payload.
 * param[out] RcvSize Pointer to the variable to store the size of response message's payload.
 * param[in] userData User data send with the message, return with the ACK/NACK feedback, can be used by application
 *                       to match feedback to the message.
 *
 * retval kIPMQ_Ret_ReceiveUnkownMsg  An unknown message was received
 ...
 * retval kIPMQ_Ret_ReceiveNACK The message was received but there was a failure,
 *          a negative acknowledgment (NACK) is returned, and the NACK reason may contain in payload.
 * retval kIPMQ_Ret_Busy One message already sent to AON but not received, please invoke IPMQ_ReceiveMessage() to 
            complete send-receive operation.
 */
ipmq_ret_value_t IPMQ_Send1PayloadMsgThenRcv(ipmq_msg_opcode_t opcode, uint8_t sendSize, uint32_t *ptrSendBuffer,
                                            uint32_t *ptrRcvBuffer, uint8_t *RcvSize, uint8_t userData)
{
    ipmq_ret_value_t ret;

    ret = IPMQ_SendMessage(opcode, userData, sendSize, ptrSendBuffer);

    if (ret != kIPMQ_Ret_SendSuccess)
    {
        return ret;
    }

    ret = IPMQ_ReceiveMessage(opcode, userData, ptrRcvBuffer, RcvSize);

    return ret;
}

/*!
 * brief Send message with two payload buffer and then waiting receive message from AON.
 * 
 * param[in] opcode Message opcode, please refer to ipmq_msg_opcode_t.
 * param[in] sendBuffer1Size Size of payload1 to send.
 * param[in] ptrSendBuffer1 Pointer to the buffer of payload1 to send.
 * param[in] sendBuffer2Size Size of payload2 to send.
 * param[in] ptrSendBuffer2 Pointer to the buffer of payload2 to send.
 * param[out] ptrRcvBuffer Pointer to the buffer to store response message's payload.
 * param[out] RcvSize Pointer to the variable to store the size of response message's payload.
 * param[in] userData User data send with the message, return with the ACK/NACK feedback, can be used by application
 *                       to match feedback to the message.
 ...
 * retval kIPMQ_Ret_Busy One message already sent to AON but not received, please invoke IPMQ_ReceiveMessage() to 
            complete send-receive operation.
 */
ipmq_ret_value_t IPMQ_Send2PayloadMsgThenRcv(ipmq_msg_opcode_t opcode, uint8_t sendBuffer1Size, 
                                            uint32_t *ptrSendBuffer1, uint8_t sendBuffer2Size,
                                            uint32_t *ptrSendBuffer2, uint32_t *ptrRcvBuffer,
                                            uint8_t *RcvSize, uint8_t userData)
{
    ipmq_ret_value_t ret;

    ret = IPMQ_SendMessage2Payload(opcode, userData, sendBuffer1Size,
                                ptrSendBuffer1, sendBuffer2Size, ptrSendBuffer2);
