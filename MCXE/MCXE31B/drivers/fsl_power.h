...
#define FSL_POWER_DRIVER_VERSION (MAKE_VERSION(2, 0, 0)) /*!< Version 2.0.0. */
/*! @} */
...
{
    kPMC_FPMVDD_HV_ALowPowerVoltageResetEventFlag =
        PMC_LVSC_LVRAF_MASK,    /*!< Low-voltage reset event on VDD_HV_A domain in FPM(full performance mode). */
    kPMC_LPMFPMVDD_HV_ALowPowerVoltageResetEventFlag =
        PMC_LVSC_LVRALPF_MASK,  /*!< Low-power reset event on VDD_HV_A domain in both LPM(low performance mode) and FPM.
                                 */
    kPMC_FPMVDD_HV_BLowPowerVoltageResetEventFlag =
        PMC_LVSC_LVRBF_MASK,    /*!< Low-voltage reset event on VDD_HV_B domain in FPM. */
    kPMC_LPMFPMVDD_HV_BLowPowerVoltageResetEventFlag =
        PMC_LVSC_LVRBLPF_MASK,  /*!< Low-power reset event on VDD_HV_B domain in both LPM and FPM. */
    kPMC_FPMV25LowPowerVoltageResetEventFlag =
        PMC_LVSC_LVR25F_MASK,   /*!< Low-voltage reset event on V25 domain in FPM. */
    kPMC_LPMFPMV25LowPowerVoltageResetEventFlag =
        PMC_LVSC_LVR25LPF_MASK, /*!< Low-power reset event on V25 domain in both LPM and FPM. */
    kPMC_FPMV11LowPowerVoltageResetEventFlag =
        PMC_LVSC_LVR11F_MASK,   /*!< Low-voltage reset event on V11 1.1V domain in FPM. */
    kPMC_LPMFPMV11LowPowerVoltageResetEventFlag =
        PMC_LVSC_LVR11LPF_MASK, /*!< Low-power reset event on V11 1.1V domain in both LPM and FPM. */
    kPMC_FPMOSCV25GoNoGoEventFlag =
        PMC_LVSC_GNG25OSCF_MASK, /*!<  Go/NoGo sensor has detected a low voltage on the V25 domain in FPM. */
    kPMC_FPMOSCV11GoNoGoEventFlag =
        PMC_LVSC_GNG11OSCF_MASK, /*!<  Go/NoGo sensor has detected a low voltage on the V11 domain in FPM. */
...
{
    kPMC_FPMVDD_HV_AHighVoltageDetectFlag =
        PMC_LVSC_HVDAF_MASK, /*!< Voltage on VDD_HV_A is above high-voltage detect threshold and FPM is active. */
    kPMC_FPMVDD_HV_BHighVoltageDetectFlag =
        PMC_LVSC_HVDBF_MASK, /*!< Voltage on VDD_HV_B is above high-voltage detect threshold and FPM is active. */
    kPMC_FPMV25HighVoltageDetectFlag = PMC_LVSC_HVD25F_MASK,  /*!< Voltage on V25 is above high-voltage detect threshold
                                                                 and full performance mode is active. */
    kPMC_FPMV11HighVoltageDetectFlag = PMC_LVSC_HVD11F_MASK,  /*!< Voltage on V11 is above high-voltage detect threshold
                                                                 and full performance mode is active. */
    ...
    kPMC_VDD_HV_AV5LowVoltageDetectFlag =
        PMC_LVSC_LVD5AF_MASK,                            /*!< Voltage on VDD_HV_A is below 5V low-voltage threshold. */
    kPMC_V15LowVoltageDetectFlag = PMC_LVSC_LVD15F_MASK, /*!< Voltage on V15 is below low-voltage detect threshold and
                                                            full performance mode is active. */
...
{
    kPMC_FPMVDD_HV_AHighVoltageStatus =
        PMC_LVSC_HVDAS_MASK,  /*!< Voltage on VDD_HV_A is above high-voltage detect threshold and FPM is active. */
    kPMC_FPMVDD_HV_BHighVoltageStatus =
        PMC_LVSC_HVDBS_MASK,  /*!< Voltage on VDD_HV_B is above high-voltage detect threshold and FPM is active. */
    kPMC_FPMV25HighVoltageStatus =
        PMC_LVSC_HVD25S_MASK, /*!< Voltage on V25 is above high-voltage detect threshold and FPM is active. */
    kPMC_FPMV11HighVoltageStatus =
        PMC_LVSC_HVD11S_MASK, /*!< Voltage on V11 is above high-voltage detect threshold and FPM is active. */
    kPMC_VDD_HV_AV5LowVoltageStatus =
        PMC_LVSC_LVD5AS_MASK, /*!< Voltage on VDD_HV_A is below low-voltage detect threshold. */
    kPMC_V15LowVoltageStatus =
        PMC_LVSC_LVD15S_MASK, /*!< Voltage on V15 is below low-voltage detect threshold and FPM is active. */
 ...
 *
 * @param externalBJTUsed Set to true if external BJT between VDD_HV_A and V15 is used,
 * otherwise set to false
 */
static inline void POWER_EnableLastMileRegulator(bool externalBJTUsed)
{
    /*
     * This field must be set to 1 if external BJT between VDD_HV_A and V15 is used on the PCB.
     * The base of this BJT must be connected to the VRC_CTRL pin and is controlled by the PMC to regulate a voltage
     * of 1.5V on V15 pin.
     */
    if (externalBJTUsed)
    {
    ...
    /* After setting LMBCTLEN=1 the software has to wait for LVD15S=0 and then setting LMEN=1 */
 ...
 *
 * @param mask    The event flags to clear. This is a logical OR of members of the
 *               enumeration ::pmc_event_flag_t
 */
 ...
 *                enumeration ::pmc_interrupt_enable_t
 */
 ...
 *
 * @param mask    The interrupts to clear. This is a logical OR of members of the
 *                enumeration ::pmc_voltage_detect_flag_t
 *
 */
 ...
 *
 * @return true if previous mode is standby mode, false if previous mode is reset mode.
 */
 ...
 */
static inline void POWER_EnterStandbyMode(void)
{
